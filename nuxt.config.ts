...
export default defineNuxtConfig({
  ...
  /**
   * * Vite Config
   * Workaround for a Windows-only Vite 6 bug where virtual module ids like
   * "virtual:public?...svg" get denied by the strict dev file-serving check
   * (the colon is mistaken for a drive letter). See ERR_DENIED_ID 500 errors.
   */
  vite: {
    server: {
      fs: {
